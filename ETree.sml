...
(` PERM (((h:('a  -> bool))::L):('a  -> bool)list) ((L1 ++ [h]):('a  -> bool)list) /\ 
   n <= LENGTH ((h::L):('a  -> bool)list) ` by (RW_TAC std_ss[])) THENL[
   (` ((h::L):'a  SET list) =  [h ]++ (L:('a  -> bool)list) ` by (RW_TAC list_ss[])) 
...
val PROB_PATH_APPEND = store_thm("PROB_PATH_APPEND",
  ``!p L2 L1. prob_space p /\ (!x. MEM x (L2++L1) ==> x IN events p ) /\ (~NULL L2) /\ 
    (~NULL L1) /\  MUTUAL_INDEP p (L2++L1) ==>
