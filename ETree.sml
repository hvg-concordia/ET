...
`MUTUAL_INDEP p (L) =
 ...
 (prob p (BIG_INTER p (TAKE n L1)) = PROD_LIST (PROB_LIST p (TAKE n L1 ))))`;
...
val PROB_BIG_INTER = store_thm("PROB_BIG_INTER",
  ``!p L. prob_space p /\ ~NULL L /\ (!x'. MEM x' L ==> x'  IN  events p ) /\
 MUTUAL_INDEP p L ==>
(prob p (BIG_INTER p L) =  PROD_LIST (PROB_LIST p L))``,
RW_TAC std_ss[] THEN
...
val BIG_UNION_DISJOINT = store_thm("BIG_UNION_DISJOINT",
``! L h p. prob_space p /\ (∀y. MEM y L ⇒ y ∈ events p) /\ ALL_DISTINCT (h::L) /\
           disjoint (h INSERT set L)  ==> (DISJOINT h (BIG_UNION p L ))``,
...
val PROB_BIG_UNION = store_thm("PROB_BIG_UNION",
``! L p. prob_space p /\ (∀y. MEM y L ⇒ y ∈ events p) /\ ALL_DISTINCT L /\ disjoint (set L) ==>
 ( prob p (BIG_UNION p L) = PROB_SUM p L)``,

rw []
\\ Induct_on `L`
   >-( rw [BIG_UNION_DEF, PROB_SUM_DEF, PROB_EMPTY])
\\ rw []
\\ rw [BIG_UNION_DEF, PROB_SUM_DEF]
\\ sg `BIG_UNION p L IN events p`
   >-( rw [BIG_UNION_IN_EVENTS])
\\ sg `DISJOINT h (BIG_UNION p L) `
   >-( rw [BIG_UNION_DISJOINT])
\\  sg `additive p `
   >-(rw [PROB_SPACE_ADDITIVE])
\\ sg `prob p (h ∪ BIG_UNION p L) = prob p h + prob p (BIG_UNION p L) `
...
val BIG_INTER_IN_EVENTS = store_thm("BIG_INTER_IN_EVENTS",
  ``!L p. (!x. MEM x L ==> x IN events p) /\ prob_space p ==> (BIG_INTER p L IN events p)``,
RW_TAC std_ss []
\\ Induct_on `L`
   >-( RW_TAC std_ss [MEM, BIG_INTER_DEF, EVENTS_SPACE])
...
val ETREE_BRANCH_MULTIPLE_EVENTS = store_thm("ETREE_BRANCH_MULTIPLE_EVENTS",
  ``! p X LN. prob_space p /\
      ...
      (prob p (ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN))) =
...
\\ sg `BIG_INTER p X ∩ h  ∈ events p`
   >-( Induct_on `X`
       ...
       \\ qpat_x_assum `(∀x'. MEM x' X ∨ (x' = h) ∨ MEM x' LN ⇒ x' ∈ events p) ⇒
         	       ...
         	       BIG_INTER p X ∩ h ∈ events p` mp_tac
       \\ ASM_REWRITE_TAC []
       \\ rw []
       \\ sg `h' ∩ BIG_INTER p X ∩ h  = h' ∩ (BIG_INTER p X ∩ h)`
       	  >-( metis_tac [INTER_ASSOC])
       \\ fs []
       \\ metis_tac [EVENTS_INTER])
\\ sg `BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN)) ∈ events p `     
   >-( pop_assum mp_tac
       \\ ntac 4 POP_ORW
       \\ ntac 2 (pop_assum mp_tac)
       \\ POP_ORW
       ...
       \\ sg ` (∀x'. MEM x' X ∨ (x' = h) ∨ MEM x' LN ⇒ x' ∈ events p)`
       	  >-( metis_tac [])
       \\ qpat_x_assum ` (∀x'. MEM x' X ∨ (x' = h) ∨ MEM x' LN ⇒ x' ∈ events p) ⇒
                         BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN)) ∈
         		 events p` mp_tac
       \\ ASM_REWRITE_TAC []
       \\ rw []
       \\ rw [EVENT_TREE_LIST_DEF]
       ...
       \\ rw [UNION_OVER_INTER]
       \\ sg `BIG_INTER p X ∩ h' ∈ events p`
          >-(metis_tac [BIG_INTER_IN_EVENTS, EVENTS_INTER])
       \\ metis_tac [EVENTS_UNION])
\\ sg `DISJOINT (BIG_INTER p X ∩ h)
      		(BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN)))`
   >-( ntac 3 POP_ORW
       \\ ntac 3 (pop_assum mp_tac)
       \\ ntac 3 POP_ORW
       ...
       \\ rw [INTER_ASSOC]
       \\ rw [UNION_OVER_INTER]
       	  >-( sg `BIG_INTER p X ∩ h ∩ BIG_INTER p X ∩ BIG_INTER p X ∩ h' =  (h ∩ h') ∩
	          BIG_INTER p X`
	          >-( rw [EXTENSION]
		      \\ EQ_TAC
		      	 >-(metis_tac [])
      ...
      \\ fs []
      \\ fs [INTER_ASSOC]
      \\ sg `BIG_INTER p X ∩ h ∩ BIG_INTER p X ∩ BIG_INTER p X ∩
      	     ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN)) =
	     BIG_INTER p X ∩ h ∩ BIG_INTER p X ∩
             ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN))`
         >-( rw [EXTENSION]
	     \\ EQ_TAC
		>-(metis_tac [])
...
\\ sg `additive p `
   >-(rw [PROB_SPACE_ADDITIVE])
\\ sg `prob p (BIG_INTER p X ∩ h ∪
       BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN)))=
      prob p (BIG_INTER p X ∩ h) + 
      prob p (BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X) (EVENT_TREE_LIST LN)))`
   >-(metis_tac [ADDITIVE_PROB])
\\ fs []
\\ rw [BRANCH_EQ_INTER_NODE]
\\ rw [INTER_ASSOC]
...
\\ ASM_REWRITE_TAC []
\\ rw []
\\ sg `BIG_INTER p X ∩ h = BIG_INTER p (h::X)`
   >-( rw [BIG_INTER_DEF]
       \\ rw [INTER_COMM])
\\ fs []      
\\ DEP_REWRITE_TAC [PROB_BIG_INTER]
...
\\ rw [INTER_ASSOC]);
(*---------------------------------------------------------------------------------------------------*)
val BRANCH_BIG_INTER_TWO_STAIR_N_STAIR = store_thm("BRANCH_BIG_INTER_TWO_STAIR_N_STAIR",
  ``ETREE (BRANCH (BIG_INTER p X) (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) =
       BIG_INTER p X ∩ ETREE (NODE (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))``, 
Induct_on `h`
>-( rw [ETREE_TWO_STAIR_DEF]
    \\ rw [ETREE_DEF])
\\ rw []
...
val DISJOINT_BIG_INTER_N_STAIR  = store_thm("DISJOINT_BIG_INTER_N_STAIR",
``!h' h LN L p X. (disjoint (h' INSERT set h)) /\ disjoint (set LN) /\ (¬MEM h' h) /\
                  EVENT_OUTCOME_SPACE_CONDS L ==>
  (DISJOINT (BIG_INTER p X ∩ h' ∩ ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))
	    (BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X)
...
\\ rw [UNION_OVER_INTER]
   >-( rw [INTER_ASSOC]
       ...
       \\ sg ` BIG_INTER p X∩ h' ∩
               ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)) ∩
   	       BIG_INTER p X ∩ BIG_INTER p X ∩ h'' ∩
   	       ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)) =
	        (h' ∩ h'') ∩ BIG_INTER p X ∩
   		ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)) ∩
   		BIG_INTER p X ∩ BIG_INTER p X ∩
   		ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))`
          >-( rw [EXTENSION]
       	      \\ EQ_TAC
       	      	 >-(metis_tac [])
...
val ETREE_BRANCH_MULTIPLE_EVENTS_N_STAIR = store_thm("ETREE_BRANCH_MULTIPLE_EVENTS_N_STAIR",
   ``!X LN L p h. prob_space p /\
    ...
    (prob p (ETREE (BRANCH (BIG_INTER p X) (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) =
   ...
   >-( fs []
       ...
       \\ qpat_x_assum `(∀x'.
              ((MEM x' X ∨ MEM x' LN) ∨ (x' = ∅) ∨ MEM x' h) ∨ MEM x' (FLAT L) ⇒
                x' ∈ events p) ⇒ (prob p  (BIG_INTER p X ∩ ETREE
                (BRANCH (BIG_INTER p X)  (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))) =
       ...
       \\ sg `prob p (BIG_INTER p X ∩ h' ∩
       	      ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)) ∪
      	      BIG_INTER p X ∩
     	      ETREE (BRANCH (BIG_INTER p X)
              (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))) =
       	      prob p (BIG_INTER p X  ∩ h' ∩
       	      ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) +
      	      prob p (BIG_INTER p X ∩
     	      ETREE (BRANCH (BIG_INTER p X)
              (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))))`	      
          >-( sg`BIG_INTER p X ∩ h' ∩
     	         ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)) ∈ events p`
	      >-(metis_tac [BIG_INTER_IN_EVENTS, EVENTS_INTER, ETREE_NODE_N_STAIR_IN_EVENTS])
              \\ sg `BIG_INTER p X ∩
	      	     ETREE (BRANCH (BIG_INTER p X)
                     (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))
		     ∈ events p`
                 >-( sg `ETREE (BRANCH (BIG_INTER p X)
                         (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))
		         ∈ events p`
			 ...
			 \\ DEP_REWRITE_TAC [ETREE_BRANCH_N_STAIR_IN_EVENTS]
		    	 \\ rw []
			 \\ metis_tac[BIG_INTER_IN_EVENTS])
	            \\ metis_tac [BIG_INTER_IN_EVENTS, EVENTS_INTER])
	      \\ sg `(DISJOINT (BIG_INTER p X ∩ h' ∩
                      ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) ( BIG_INTER p X ∩
      		      ETREE (BRANCH (BIG_INTER p X)
      		      (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))))) `
		 >-( DEP_REWRITE_TAC [DISJOINT_BIG_INTER_N_STAIR]
		    \\ fs[]
		    \\ fs[disjoint]
		    \\ metis_tac [])
             \\ metis_tac [ADDITIVE_PROB])
	...
	\\ sg `(∀x'. (((x' = h') ∨ MEM x' X) ∨ MEM x' LN) ∨ MEM x' (FLAT L) ⇒
       ...
       \\ qpat_x_assum `(∀x'. (((x' = h') ∨ MEM x' X) ∨ MEM x' LN) ∨ MEM x' (FLAT L) ⇒
               		...
               		(BRANCH (BIG_INTER p (h'::X)) (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) =
       ...
       \\ REAL_ARITH_TAC)
\\ sg `prob p (BIG_INTER p X ∩ h' ∩ ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)) ∪
       BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X)
       (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))) =
       prob p (BIG_INTER p X  ∩ h' ∩ ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) +
       prob p (BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X)
       (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))))`	      
   >-( sg`BIG_INTER p X ∩ h' ∩ ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)) ∈ events p`
       >-(metis_tac [BIG_INTER_IN_EVENTS, EVENTS_INTER, ETREE_NODE_N_STAIR_IN_EVENTS])
       \\ sg `BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X)
          ...
          >-( sg `ETREE (BRANCH (BIG_INTER p X)
		   ...
		   \\ metis_tac[BIG_INTER_IN_EVENTS])
	       \\ metis_tac [BIG_INTER_IN_EVENTS, EVENTS_INTER])
      \\ sg `(DISJOINT (BIG_INTER p X ∩ h' ∩ ETREE (NODE (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))
             (BIG_INTER p X ∩ ETREE (BRANCH (BIG_INTER p X)
      	     (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))))) `
	 >-( DEP_REWRITE_TAC [DISJOINT_BIG_INTER_N_STAIR]
	     \\ fs[]
...
\\ qpat_x_assum `(∀x'. ((MEM x' X ∨ MEM x' LN) ∨ MEM x' h) ∨ MEM x' (FLAT L) ⇒
                    x' ∈ events p) ⇒ (prob p (ETREE (BRANCH (BIG_INTER p X)
                    (ETREE_TWO_STAIR h (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))) =
          	    PROD_LIST (PROB_LIST p X) * PROB_SUM p LN * PROB_SUM p h *
          	    PROD_LIST (PROB_SUM_LIST p L))` mp_tac 
\\ ASM_REWRITE_TAC []
...
\\ rw [INTER_COMM]
\\ rw [ GSYM BIG_INTER_DEF]
\\ rw [BRANCH_BIG_INTER_TWO_STAIR_N_STAIR]
\\ rw [INTER_ASSOC]
\\ rw [GSYM BRANCH_BIG_INTER_TWO_STAIR_N_STAIR]
\\ ntac 5 POP_ORW
\\ first_x_assum (mp_tac o Q.SPECL [`h'::X`])
\\ rw []
\\ first_x_assum (mp_tac o Q.SPECL [`LN`])
...
\\ fs []
\\ qpat_x_assum `(∀x'.(((x' = h') ∨ MEM x' X) ∨ MEM x' LN) ∨ MEM x' (FLAT L) ⇒
                 x' ∈ events p) ⇒ (prob p (ETREE (BRANCH (BIG_INTER p (h'::X))
                 (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) =
          	 PROD_LIST (PROB_LIST p (h'::X)) * PROB_SUM p LN *
          	 PROD_LIST (PROB_SUM_LIST p L))` mp_tac 
\\ ASM_REWRITE_TAC []
...
\\ rw []
   >-( rw []
	...
	\\ sg `prob p (ETREE (BRANCH h' (ETREE_N_STAIR (EVENT_TREE_LIST LN) L))) =
	       prob p (ETREE (BRANCH (BIG_INTER p [h']) (ETREE_N_STAIR (EVENT_TREE_LIST LN) L)))`
	   >-(metis_tac [BIG_INTER_DEF, INTER_PSPACE, INTER_COMM])
	\\ fs []
...
val _ = Unicode.unicode_version {tmnm = "CUSTOMER_INTERRUPTIONS",
                                 u = SUM^LIGHTNING};
...
fun HOL_TO_REAL_EXP_EVALUATION t =
 let
    val failstring = "Symbolic expression could not be translated in a number"
 in
    case strip_comb t of (f,[a1,a2]) =>  

       HOL_TO_REAL f (HOL_TO_REAL_EXP_EVALUATION a1,HOL_TO_REAL_EXP_EVALUATION a2)
       | (f,[a]) =>
                if f ~~ ``($&):num -> real`` then Arbnum.toReal (dest_numeral a)
...
fun PROBABILITY T X =
let 
    val value = HOL_TO_REAL_EXP_EVALUATION  X;
in
...
let
    val value = HOL_TO_REAL_EXP_EVALUATION  X;    
in
    print("Total Annual System Customer Interruptions in 1 Year" ^ " " ^ "Equal" ^ " ");
    print(Real.toString (value) ^ "\n\n")
end;
(*--------------------------------------------------------------------------------------------------*)
...
let
    val value = HOL_TO_REAL_EXP_EVALUATION  X;
in
    print("System Average Interuption Frequency Index (SAIFI) in 1 Year (Interruptions/System Customer)" ^ " " ^ "Equal" ^ " ");
    print(Real.toString (value) ^ "\n\n")
end;
