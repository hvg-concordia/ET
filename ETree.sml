...
(* File Name: ETree.sml	          	                                     *)
(*---------------------------------------------------------------------------*)
(*          Description: Event Tree Reilability Analysis of  Safety          *)
(*	    		 Critical Systems using Theorem Proving	     	     *)
(*                            	  			     		     *)
(*          HOL4-Kananaskis 13 		 			     	     *)
(*									     *)
...
(*--------------------------------------------------------------------------------------------------*)
		...
		(*--------------------------------------------------------*)
     		(* Power System Generic Reliability and Energy Indices    *)
     		(*--------------------------------------------------------*)
...
val PROB_ET_X_DEF = Define
`PROB_ET_X LN L Ns CEs E p = prob p (ETREE (NODE (E ⊞ (☒Ν (⊗Ν𝑳 LN L) Ns CEs))))`;
(*--------------------------------------------------------------------------------------------------*)

val LOAD_INTERRUPTIONS_DEF = Define
`(LOAD_INTERRUPTIONS LN L Ns CEs [] [] p = 0) /\ 
 (LOAD_INTERRUPTIONS LN L Ns CEs (E::Es) (CN::CNs) p =
 (PROB_ET_X  LN L Ns CEs E p) * (&CN) + (LOAD_INTERRUPTIONS LN L Ns CEs Es CNs p))`;

val LIGHTNING  =  U 0x021AF;
val _ = Unicode.unicode_version {tmnm = "LOAD_INTERRUPTIONS", u = SUM^L^O^A^D^LIGHTNING};

(*where*)
(* LN   : Last power system component modes*)
(* L    : list of power system components modes*)
(* Ns   : list of complete cylinder path numbers*)
(* CEs  : list of complete cylinder conditional events*)
(* Es   : list of partitioning pathes of certian consequence events*) 
(* CNs  : list of customer numbers affected by Es*)
(*--------------------------------------------------------------------------------------------------*)
...
`SAIFI LN L Ns CEs Es CNs p = (∑𝑳𝑶𝑨𝑫↯  LN L Ns CEs Es CNs p) / (&SUM CNs)`;
(*--------------------------------------------------------------------------------------------------*)
...
`(LOAD_TIME_INTERRUPTIONS LN L Ns CEs [] [] [] p = 0) /\ 
 ...
 (PROB_ET_X  LN L Ns CEs E p) * MTTR * (&CN) + (LOAD_TIME_INTERRUPTIONS LN L Ns CEs Es MTTRs CNs p))`;
...
`SAIDI LN L Ns CEs Es MTTRs CNs p = (∑𝑻𝑳𝑶𝑨𝑫↯ LN L Ns CEs Es MTTRs CNs p) / &(SUM CNs)`;
(*--------------------------------------------------------------------------------------------------*)
...
`CAIDI LN L Ns CEs Es MTTRs CNs p  = (SAIDI LN L Ns CEs Es MTTRs CNs p)/(SAIFI LN L Ns CEs Es CNs p)`;
(*--------------------------------------------------------------------------------------------------*)
...
val ASAI_DEF = Define
`ASAI LN L Ns CEs Es MTTRs CNs p  =
((&(SUM CNs) * 8760) - ∑𝑻𝑳𝑶𝑨𝑫↯ LN L Ns CEs Es MTTRs CNs p)/(&(SUM CNs) * 8760)`;
(*--------------------------------------------------------------------------------------------------*)
...
val ASUI_DEF = Define
`ASUI LN L Ns CEs Es MTTRs CNs p  = (∑𝑻𝑳𝑶𝑨𝑫↯ LN L Ns CEs Es MTTRs CNs p)/(&(SUM CNs) * 8760)`;
(*--------------------------------------------------------------------------------------------------*)

				(*-----------------------------*)
				(* Power System Energy Indices *)
				(*-----------------------------*)
...
val ENS_DEF = Define
`(ENS [] LN L Ns CEs [] [] p = 0) /\ 
 (ENS (La::LaN) LN L Ns CEs (E::Es)  (MTTR::MTTRs) p =
 La * (PROB_ET_X  LN L Ns CEs E p) * MTTR + (ENS LaN LN L Ns CEs Es MTTRs p))`;
(*--------------------------------------------------------------------------------------------------*)
...
val ASCI_DEF = Define
`ASCI LaN LN L Ns CEs Es CNs MTTRs p = (ENS LaN LN L Ns CEs Es MTTRs p)/(&(SUM CNs))`;
(*--------------------------------------------------------------------------------------------------*)
...
val LOLE_DEF = Define
`(LOLE LN L Ns CEs [] [] p = 0) /\ 
 (LOLE LN L Ns CEs (E::Es)  (tk::tks) p =
(PROB_ET_X  LN L Ns CEs E p) * tk + (LOLE LN L Ns CEs Es tks p))`;
(*--------------------------------------------------------------------------------------------------*)
...
val LOEE_DEF = Define
`(LOEE LN L Ns CEs [] [] p = 0) /\ 
 (LOEE LN L Ns CEs (E::Es) (Ek::Eks) p =
(PROB_ET_X  LN L Ns CEs E p) * Ek + (LOEE LN L Ns CEs Es Eks p))`;
(*--------------------------------------------------------------------------------------------------*)
...
val EIR_DEF = Define
`EIR  LN L Ns CEs Es ET Eks p  = 1 - (LOEE LN L Ns CEs Es Eks p)/(ET)`;
(*--------------------------------------------------------------------------------------------------*)
(*--------------------------------------------------------------------------------------------------*)
(*--------------------------------------------------------------------------------------------------*)
(*--------------------------------------------------------------------------------------------------*)
