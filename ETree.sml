...
val RELIABILITY_DEF = Define` RELIABILITY p X t =  distribution p X {y |  Normal t < y}`;
...
` (EXP_ET_SUCCESS_DISTRIB_LIST p [] [] = T) /\
  (EXP_ET_SUCCESS_DISTRIB_LIST p (h::t) (x::xs) <=> (EXP_ET_SUCCESS_DISTRIB p h x) /\
                                                     EXP_ET_SUCCESS_DISTRIB_LIST p t xs) `;
...
\\ sg ` MUTUAL_INDEP p (SUCCESS p LN t::FAIL p LN t::
   ...
   >-( once_rewrite_tac[(prove(``!a b c. a::b::c = [a;b]++c``,rw[]))]
       ...
       \\ irule MUTUAL_INDEP_APPEND1 
       \\ rw[])
\\ sg `∏ (∑𝟚𝗗𝑷𝑹𝑶𝑩 p (SUCCESS_FAIL_LIST p L t)) = 1 `
   >-( metis_tac [])
\\ fs []
\\ REAL_ARITH_TAC);
(*--------------------------------------------------------------------------------------------------*)
...
(GRID_INTERRUPTIONS LN_ALL L_ALL Ns_ALL CEs_ALL Es_ALL CNs_ALL p)/ (&SUM_CUSTOMER CNs_ALL)`;
(*--------------------------------------------------------------------------------------------------*)
(*--------------------------------------------------------------------------------------------------*)
...
`(LOAD_TIME_INTERRUPTIONS LN L Ns CEs [] [] [] p = 0) /\ 
 (LOAD_TIME_INTERRUPTIONS LN L Ns CEs (E::Es)  (MTTR::MTTRs) (CN::CNs) p =
  (\a b c. prob p (ETREE (NODE (EVENT_TREE_LIST (PARTITIONING a
	 (NESTED_COMPLETE_CYLINDER (ETREE_N_STAIR_ALL_PATHS LN L) Ns CEs))))) * b * c) E (&CN) MTTR
...
(GRID_TIME_INTERRUPTIONS LN_ALL L_ALL Ns_ALL CEs_ALL Es_ALL MTTRs_ALL CNs_ALL p)/ (&SUM_CUSTOMER CNs_ALL)`;
(*--------------------------------------------------------------------------------------------------*)
(*--------------------------------------------------------------------------------------------------*)
